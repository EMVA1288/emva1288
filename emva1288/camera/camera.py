...
class Camera(object):
    ...
    def __init__(self,
        ...
        if bayer:
            self.bayer = self._u_bayer(bayer_filter_type,
                                       transmition_green,
                                       transmition_blue,
                                       transmition_red)
        else:
    ...
    def _u_bayer(self, pattern, t_g, t_b, t_r):
        ...
        Raises
        ------
        ValueError: An error generate by the value of the pattern if is not
                    0, 1, 2 or 3.
        """
        if pattern == 0:
            ...
            raise ValueError('%s is not a bayer filter option! (0,1,2,3)'
                             % (pattern))
        h = self.height
        w = self.width
        size = (int(np.ceil(h/pattern_rep.shape[1])),
                int(np.ceil(w/pattern_rep.shape[0])))
