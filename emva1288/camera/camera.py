...
class Camera(object):
    ...
    def __init__(self,
        ...
        electron_saturation : int64, optional
            The physical saturation of a pixel with electrons (full-well capacity)
        ...
        self._electron_saturation = electron_saturation
    ...
    def grab(self, radiance, temperature=None, f_number=None):
        ...
        electron_img = self._rng.poisson(u_d, size=self._shape)
        ...
        electron_img += self._rng.poisson(u_e, size=self._shape)
        ####################################################################
        ...
        dark_signal = self._dsnu + self._rng.normal(loc=self._dark_signal_0,
                                                    scale=variance,
                                                    size=self._shape)
        electron_img = electron_img + dark_signal

        ###########################################
        # Clip of the Full well electrons capacity
        np.clip(electron_img, 0, self._electron_saturation, electron_img)

        # Analog to Digital gain
        img = self.K * electron_img
